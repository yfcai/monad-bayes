...
mhKernel program = MHKernel $ fmap fst . flip runStateT mempty . runMHKernel (mhKernelWithReuseLog program)
...
mhKernelWithReuseLog' :: (RandomDB r, MonadDist m) =>
                         r -> UpdaterT r m a -> MHKernel (StateT MHReuseLog m) (a, r)
mhKernelWithReuseLog' = const mhKernelWithReuseLog

mhKernelWithReuseLog :: (RandomDB r, MonadDist m) =>
                        UpdaterT r m a -> MHKernel (StateT MHReuseLog m) (a, r)
mhKernelWithReuseLog program = MHKernel $ \(x, r) -> do
  r1 <- lift $ mutate r
  ((x', r'), leftover) <- lift $ runUpdaterT program r1
  modify $ mappend $ MHReuseLog (size r) (size r') (size $ resampled r r')
...
mhWithAcceptLog :: forall m a. (MonadDist m) =>
                   Int ->  WeightedT m a -> MHKernel (WeightedT m) a -> StateT MHAcceptLog m [a]
mhWithAcceptLog n init trans = evalStateT (start >>= chain n) 1 where
  start :: StateT LogFloat (StateT MHAcceptLog m) a
  start = do
    (x, p) <- lift $ lift $ runWeightedT init
    put p
    return x

  chain :: Int -> a -> StateT LogFloat (StateT MHAcceptLog m) [a]
  chain 0 _ = return []
  chain n x = do
    p <- get
    ...
    when accept (put q)
    lift $ modify $ mappend $ MHAcceptLog (if accept then 1 else 0) 1 (fromLogFloat ratio)
    rest <- chain (n-1) next
    return (x:rest)
...
mh n init trans = fmap fst $ runStateT (mhWithAcceptLog n init trans) mempty
